...
const regex = /\/\/\s*3\. Buscar el Lead Magnet solicitado[\s\S]*?UPDATE downloads SET sent = true WHERE id = \$1', \[downloadId\]\);\n\s+\}/;
...
const replacement = `        // 3. Buscar la configuración del Recurso en la página (site_nodes)
        ...
        if (!emailSubject || !emailBody || !fileUrl) {
            throw new Error("El correo de este recurso no se ha configurado en Godzilla Studio -> Recursos -> 💌 Correos.");
        }

        // 5. Registrar la descarga (insertamos temporalmente sin lead_magnet_id)
        ...
        // 6. ENVIAR CORREO usando la API de Resend / Brevo
...
console.log("leadController.js patched successfully!");
