...
export default function AdminStudio() {
 ...
 <div className="p-3 border-t border-[#CC0000]/40 space-y-1 bg-black/30">
   ...
   className={`w-full p-2 rounded-xl border border-transparent transition-colors flex items-center gap-3 shadow-sm ${ activeSection ==='profile' ?'bg-black/50 border-[#CC0000]/50' :'hover:bg-black/60 hover:border-red-900/30 bg-[#CC0000]/5' }`}>
  ...
  📧 Newsletter
   ...
   {adminProfile?.role === 'cm' || adminProfile?.username?.toLowerCase() === 'judith' ? (
       ...
       <button onClick={() => { setIsAnalyticsMode(false); setActiveSection('social'); setSelectedNodeId(null); navigate('/cm'); }}
       className={`w-full text-[10px] py-2 shadow-sm mb-2 rounded-xl transition-all font-black uppercase flex items-center justify-center border ${ activeSection ==='social' ?'bg-white/70 text-[#CC0000] border-[#CC0000]/50' :'bg-[#CC0000]/5 text-neutral-300 border-[#CC0000]/40 hover:bg-black/50 hover:text-white' }`}>
 ...
 className="w-full text-[10px] text-red-500 font-bold hover:text-red-600 hover:bg-black/60 border border-transparent hover:border-red-200 py-2 rounded-xl transition-all shadow-sm">
 🚪 Cerrar sesión
 </button>
 </div>
 </div>
