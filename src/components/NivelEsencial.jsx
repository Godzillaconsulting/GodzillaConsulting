...
const defaultContent = {
    ...
    heroTitle: 'NIVEL\nESENCIAL',
    heroDisclaimer: 'Si en <span className="font-bold text-white not-italic">7 días hábiles</span>, tu sistema no está instalado, respondiendo mensajes y capturando datos de tus clientes automáticamente, te regalamos el siguiente mes de servicio completamente <span className="font-bold text-white not-italic">GRATIS</span>.',
    planTarget: 'Ideal para el que no confía y se ha vuelto esclavo de su propio éxito',
...
const NivelEsencial = () => {
    ...
    useEffect(() => {
        ...
        client.fetch(`*[_type == "landingPaquete" && slug.current == "nivel-esencial"][0]{..., "videoFileUrl": videoFile.asset->url}`)
            .then((data) => {
                if (data) {
