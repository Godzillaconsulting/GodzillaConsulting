...
router.post('/tasks', verifyJWT, verifyRole('jareg', 'judith'), async (req, res) => {
    try {
        const { title, prompt, assigned_to, tags, priority, content_type } = req.body;
        
        const query = `
            INSERT INTO studio_tasks (title, prompt, assigned_to, tags, priority, content_type)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *;
        `;
...
router.put('/tasks/:id', verifyJWT, async (req, res) => {
    try {
        ...
        // Solo actualizar los campos que el cuerpo manda explícitamente
...
// POST: Goyi Learning (Alimentar DB de re-prompts de usuario)
router.post('/learning', verifyJWT, async (req, res) => {
    try {
