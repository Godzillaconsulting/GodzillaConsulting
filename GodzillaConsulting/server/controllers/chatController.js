...
export const processChatMessage = async (req, res) => {
    ...
    try {
        ...
        const dateInjectedPrompt = SYSTEM_PROMPT.replace("## IDENTIDAD Y CONTEXTO", `## FECHA ACTUAL DEL SISTEMA\n¡ATENCIÓN! Hoy es ${sysDate} (Hora de Juárez). Utiliza esta fecha como tu presente absoluto y NUNCA asumas otra.\n\n## IDENTIDAD Y CONTEXTO`);

        const model = genAI.getGenerativeModel({
            model: "gemini-2.0-flash",
            systemInstruction: dateInjectedPrompt,
        ...
        if (functionCalls && functionCalls.length > 0) {
            for (const call of functionCalls) {
                ...
                if (call.name === "check_availability") {
                    ...
                    // Parse day to check Sunday
                    const safeDateString = `${fecha}T12:00:00`; // Mediodía para evitar saltos de zona al extraer día
