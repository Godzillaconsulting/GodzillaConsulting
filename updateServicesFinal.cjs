...
const servicesData = {
    'ProduccionAudiovisual.jsx': {
        ...
        accordionItems: `    const accordionItems = [
...
const accordionUI = `                            {/* SUBTITLE REPLACED BY ACCORDION */}
...
workspaces.forEach(ws => {
  ...
  Object.entries(servicesData).forEach(([file, data]) => {
      ...
      // 1. Remove \r entirely internally to process easily
      ...
      if (!content.includes('accordionItems')) {
          ...
          // useState & accordion items block
          const oldState = '    const [content, setContent] = useState(defaultContent);';
          const newState = oldState + '\n    const [openAccordion, setOpenAccordion] = useState(0);\n\n' + data.accordionItems;
          content = content.replace(oldState, newState);

          // Replace paragraph containing subtitle correctly using relaxed regex to handle newlines
          const pRegex = /<p className="text-white text-lg md:text-xl mb-10 leading-relaxed font-medium">\n\s*\{content\.subtitle\}\n\s*<\/p>/;
          content = content.replace(pRegex, accordionUI);

          // Add \r back if Windows line endings
          content = content.replace(/\n/g, '\r\n');
  ...
  if (fs.existsSync(adminPath)) {
      ...
      adminContent = adminContent.replace(/\r/g, '');
