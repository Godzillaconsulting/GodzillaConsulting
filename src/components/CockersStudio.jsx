...
const RAW_PROMPTS = [
    ...
    { prompt: 'Epic aerial planet view from space, milky way background, Unreal Engine 5, volumetric clouds, 8k', tag: 'Space Epic', model: 'Veo 3' },
    ...
    { prompt: 'Aerial drone shot over icy mountains at golden hour, volumetric rays, unreal engine, national geographic style', tag: 'Drone Épico', model: 'Veo 3' },
    ...
    { prompt: 'Majestic mountain landscape, 35mm analog film photography, natural golden hour, highly textured raw photo', tag: '35mm Film', model: 'Veo 3' },
    ...
    { prompt: 'Macro shot of a glowing neon jellyfish in deep dark ocean, bioluminescence, highly detailed, national geographic 8k', tag: 'Bioluminescence', model: 'Gemini 3.5 Pro' },
    { prompt: 'Synthwave outrun grid sunset, glowing magenta sun, vectorized retro horizon, 80s arcade style aesthetics', tag: 'Synthwave 80s', model: 'Gemini 3.5 Flash' },
    { prompt: 'Mystical dark forest with glowing mystical mushrooms, ethereal blue fog, cinematic fantasy concept art, highly detailed', tag: 'Dark Fantasy', model: 'Veo 3' },
    ...
    { prompt: 'Hardware motherboard glowing with neon blue and pink laser data streams, cyberpunk tech core, macro electronics', tag: 'Tech Core', model: 'Gemini 3.5 Pro' },
    ...
    { prompt: 'Cinematic shot of a neon glowing samurai standing in a rainy tokyo street, cyberpunk aesthetic, 8k resolution', tag: 'Neon Samurai', model: 'Veo 3' },
    { prompt: 'Ultra realistic macro shot of an eye with a galaxy reflecting in the pupil, cosmic lighting, 8k', tag: 'Cosmic Eye', model: 'Gemini 3.5 Pro' },
    ...
    { prompt: 'A massive floating sci-fi city above the clouds, god rays, epic scale, concept art', tag: 'Cloud City', model: 'Veo 3' },
    { prompt: 'Gothic cathedral interior lit by bioluminescent plants, eerie green glow, highly detailed', tag: 'Bio Gothic', model: 'Gemini 3.5 Flash' },
    ...
    { prompt: 'Neon signs reflected in a puddle, dark rainy night, cinematic lighting, 35mm lens', tag: 'Neon Puddle', model: 'Veo 3' },
    { prompt: 'A black hole eating a star, interstellar gargantua style, realistic physics render, 8k', tag: 'Black Hole', model: 'Gemini 3.5 Pro' },
    ...
    { prompt: 'A futuristic android repairing itself, Sparks flying, cinematic blue and orange lighting', tag: 'Android Fix', model: 'Veo 3' },
    ...
    { prompt: 'Vaporwave marble statue with pink and cyan grid, palm trees, 80s aesthetic', tag: 'Vaporwave', model: 'Gemini 3.5 Flash' },
    { prompt: 'A glowing crystal cave with underground river, bioluminescent blue water, fantasy art', tag: 'Crystal Cave', model: 'Veo 3' },
    { prompt: 'Close up of a DJ turntable with neon soundwaves bursting out, energetic club vibe', tag: 'Neon DJ', model: 'Imagen 4 Ultra' },
    { prompt: 'A cute robot dog playing with a glowing ball, futuristic living room, pixar style 3d render', tag: 'Robo Dog', model: 'Gemini 3.5 Pro' },
    ...
    { prompt: 'Macro of a frozen soap bubble with ice crystals forming, winter magic, 8k resolution', tag: 'Frozen Bubble', model: 'Veo 3' },
    { prompt: 'A futuristic bullet train speeding through a neon megacity, motion blur, cyberpunk', tag: 'Bullet Train', model: 'Gemini 3.5 Flash' },
...
export default React.memo(function CockersStudio({ adminProfile, forceOpenEditor = false }) {
    ...
    // Nuevos states: fecha de publicación, nicho, y trends del día
    const [videoManualDate, setVideoManualDate] = useState(() => new Date().toISOString().split('T')[0]);
    ...
    // Carga trends del día desde /api/trends filtrando por nicho
    const fetchVideoTrends = async () => {
        if (!videoNiche.trim()) return alert('Escribe primero el nicho/tema del video.');
        setLoadingTrends(true);
        setVideoTrends(null);
        try {
            const token = localStorage.getItem('adminToken');
            const res = await fetch(`/api/trends?network=TikTok&filter=${encodeURIComponent(videoNiche)}`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
            const data = await res.json();
            if (data.success && data.data) setVideoTrends(data.data);
            else setVideoTrends({ error: data.error || 'Sin datos de trends.' });
        } catch (e) {
            setVideoTrends({ error: e.message });
        }
    ...
    const submitVideoScript = async () => {
        ...
        try {
            ...
            alert(`Error al enviar al Estudio IA: ${e.message}`);
        } finally {
